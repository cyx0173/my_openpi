...
run_one_precision() {
...

  {
    uv run python experiments/mode1_baseline.py \
      ...
      --args.episode-end 100 \
      --args.base-dir "$BASE_DIR/$task_name/$tag"
  } >> "$LOG_DIR/${tag}_${task_name}.log" 2>&1 &

  echo $!
}
# 顺次执行 8, 6, 4, 2, 0 任务（如果想正序，改成 0 2 4 6 8 即可）
for CURRENT_ID in 8 6 4 2 0; do

    get_task_name "$CURRENT_ID"

    run_one_precision 8005 w4a4   "$CURRENT_ID" "$TASK_NAME"
    run_one_precision 8006 w4a8   "$CURRENT_ID" "$TASK_NAME"
    run_one_precision 8007 w4a16  "$CURRENT_ID" "$TASK_NAME"
